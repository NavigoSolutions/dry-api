export class ClearanceValidator {
    ...
    static isFunctionWithArityBetween(func, arityMin, arityMax, message = undefined) {
        ...
        if (func.length < arityMin || func.length > arityMax) {
            throw new Error(message || `Expected function with arity between ${arityMin} and ${arityMax} but got ${func.length}`)
        }
    }
    ...
    static arrayOfStrings(array, message = undefined) {
        ...
        array.forEach((item, index) => {
            ...
            if (item.constructor !== String) {
                throw new Error(message || `Expected string but got ${object.constructor.name} on index ${index} in array [${array.join(', ')}]`)
            }
        })
    }
