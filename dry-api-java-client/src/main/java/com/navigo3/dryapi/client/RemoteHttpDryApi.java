...
public class RemoteHttpDryApi {
	...
	public <TInput, TOutput> CompletableFuture<ValidationData> validateAsync(MethodInterface<TInput, TOutput> method,
		TInput input, Consumer<ValidationData> onSuccess) {
	...
	public <TInput, TOutput> ValidationData validateBlocking(MethodInterface<TInput, TOutput> method, TInput input) {
		...
		ValidationData resVal = ExceptionUtils.withRuntimeException(() -> validateAsync(method, input, (res) -> {
		}).get());
	...
	public <TInput, TOutput> CompletableFuture<TOutput> executeAsync(MethodInterface<TInput, TOutput> method,
		TInput input, Consumer<TOutput> onSuccess) {
	...
	public <TInput, TOutput> TOutput executeBlocking(MethodInterface<TInput, TOutput> method, TInput input) {
		...
		TOutput resVal = ExceptionUtils.withRuntimeException(() -> executeAsync(method, input, (res) -> {
		}).get());
	...
	public <TInput, TOutput> CompletableFuture<RequestData<TInput, TOutput>> callSimple(
			...
			httpClient.newCall(request).enqueue(new Callback() {
				...
				public void onResponse(Call call, Response httpResponse) throws IOException {
					...
					try {
						...
						requestsBatch.getOnFail()
							.orElse(settings.getGlobalErrorHandler().orElse(Throwable::printStackTrace));
