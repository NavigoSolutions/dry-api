...
public class JsonExecutor<TAppContext extends AppContext, TCallContext extends CallContext, TValidator extends Validator> {
	...
	public JsonBatchResponse execute(TAppContext appContext, JsonBatchRequest batch, ObjectMapper objectMapper) {
		...
		appContext.transaction(() -> {
			...
			ImmutableJsonBatchResponse res = builder.build();
			result.set(res);
			...
			if (!problems.isEmpty()) {
				appContext.reportException(new RuntimeException(problems.stream().collect(Collectors.joining("\n"))));
			}

			return res.getOverallSuccess();
		});
	...
	private void findMethod(TAppContext appContext, JsonRequest request, Builder outputBuilder,
		Consumer<MethodDefinition> onSuccess) {
